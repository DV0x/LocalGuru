...
# Script to test the multi-strategy search with a meetup query - only using autodetected intent

echo "======================================================"
echo "🧪 TESTING MEETUP QUERY WITH AUTO-DETECTED INTENT ONLY"
echo "======================================================"
...
OUTPUT_FILE="meetup_autodetect_results.txt"
QUERY="Want to meet people who dont have kids"
...
curl -s -X POST "$SUPABASE_URL/functions/v1/query-analysis" \
  ...
  -d "{\"query\":\"$QUERY\"}" > meetup_analysis.json

# Extract intent, topics, and locations
INTENT=$(jq -r '.intent' meetup_analysis.json)
TOPICS=$(jq -r '.topics[]' meetup_analysis.json 2>/dev/null || echo "")
LOCATIONS=$(jq -r '.locations[]' meetup_analysis.json 2>/dev/null || echo "")
...
curl -s -X POST "$SUPABASE_URL/functions/v1/query-embeddings" \
  ...
  -H "Content-Type: application/json" \
  -d "{\"query\":\"$QUERY\"}" > meetup_embedding.json

# Extract embedding
EMBEDDING=$(jq -r '.embedding' meetup_embedding.json)
...
cat > /tmp/test_meetup_intent.sql << EOF
...
psql "$DB_URL" -f /tmp/test_meetup_intent.sql > /tmp/meetup_intent_results.txt

# Add results to output file
echo -e "\n=== RESULTS WITH AUTO-DETECTED INTENT: $INTENT ===" >> "$OUTPUT_FILE"
cat /tmp/meetup_intent_results.txt >> "$OUTPUT_FILE"

# Count results
RESULT_COUNT=$(grep -v "^-" /tmp/meetup_intent_results.txt | grep -v "rows)" | grep "|" | wc -l)
