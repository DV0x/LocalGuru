-- Fix vector type specification by using fully qualified vector reference
...
DECLARE
  ...
  cached_embedding public.vector(1536);
  cache_key text;
...
BEGIN
  ...
  IF input IS NULL OR length(trim(input)) = 0 THEN
    ...
    RETURN array_fill(0.0, ARRAY[1536])::public.vector(1536);
  END IF;
  ...
  BEGIN
    ...
    embedding_vector := edge_function_embedding::text::float[]::public.vector(1536);
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    SELECT
      ...
      )::public.vector(1536) INTO embedding_vector
