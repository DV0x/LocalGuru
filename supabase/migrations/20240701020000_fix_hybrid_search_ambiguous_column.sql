-- Migration to fix ambiguous column references in the hybrid_search function
-- This resolves the error: "column reference "content_type" is ambiguous"
...
BEGIN
  ...
  vector_search AS (
    SELECT 
      ts.parent_id,
      ts.content_type AS result_content_type, -- FIX: Renamed to avoid ambiguity
      ts.title,
      ts.content,
      ts.url,
      ts.subreddit,
  ...
  combined_scores AS (
    SELECT
      parent_id,
      result_content_type AS content_type, -- FIX: Use renamed column
      title,
      content,
      url, 
      subreddit,
...
COMMENT ON FUNCTION public.hybrid_search IS 'Hybrid search function that combines PostgreSQL full-text search with vector similarity search. Fixed to properly handle column references, parameter interpolation, and ambiguous column names.';
