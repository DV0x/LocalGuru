...
-- Description: Updates search functions to return full content in snippets
  ...
  BEGIN
    ...
    WITH results AS (
      SELECT
        ...
        c.content AS comment_snippet, -- Return full content instead of truncating
...
COMMENT ON FUNCTION public.comment_only_search_with_timeout IS 'Timeout-resilient comment search function with fallback to text search (returns full content)';
