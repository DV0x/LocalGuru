-- Updated production-ready generate_embeddings function that delegates to edge function
...
-- Set API key if needed and if it doesn't exist
DO $$
BEGIN
  -- Only set if doesn't exist and we're in development (set your actual key in production)
  IF NOT EXISTS (SELECT 1 FROM pg_settings WHERE name = 'app.settings.openai_key' AND setting IS NOT NULL) THEN
    -- Replace with your actual key or pull from environment
    PERFORM set_config('app.settings.openai_key', 'your-openai-key-here', false);
  END IF;
EXCEPTION WHEN OTHERS THEN
  RAISE NOTICE 'Could not set OpenAI key: %', SQLERRM;
END;
$$; 