-- Migration to fix the ambiguous author column reference in hybrid_search
-- This resolves the error: "column reference "author" is ambiguous"

-- Fix the hybrid_search function by properly qualifying the author column
...
BEGIN
  ...
  WITH text_search AS (
    ...
    SELECT 
      ...
      p.url AS result_url,
      p.subreddit AS result_subreddit,
      p.author_id AS result_author,   -- FIX: Renamed to avoid ambiguity
    ...
    SELECT 
      ...
      p.url AS result_url,
      p.subreddit AS result_subreddit,
      c.author_id AS result_author,   -- FIX: Renamed to avoid ambiguity
  ...
  vector_search AS (
    SELECT 
      ts.parent_id,
      ts.content_type AS result_content_type,
      ts.result_title,
      ts.result_content,
      ts.result_url,
      ts.result_subreddit,
      ts.result_author,               -- FIX: Use renamed column
      ts.result_created_at,
      CASE
  ...
  combined_scores AS (
    SELECT
      ...
      result_author AS author,         -- FIX: Use renamed column
...
COMMENT ON FUNCTION public.hybrid_search IS 'Hybrid search function that combines PostgreSQL full-text search with vector similarity search. Fixed to properly handle column references, parameter interpolation, and all ambiguous column names including author.';
