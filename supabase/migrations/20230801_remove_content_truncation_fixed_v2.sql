...
-- This migration removes the 300 character limit on content_snippet and fixes the ambiguous similarity column reference
...
BEGIN
  ...
  ranked_results AS (
    SELECT 
      ...
      result_content_type,
      result_created_at,
      result_match_score AS result_similarity, -- FIXED: Renamed to avoid ambiguity
      result_match_type,
      result_metadata,
  ...
  SELECT 
    ...
    result_similarity AS similarity, -- FIXED: Using the renamed column to avoid ambiguity
  ...
  WHERE 
    rank = 1
  ORDER BY 
    result_similarity DESC -- FIXED: Using the renamed column consistently
  LIMIT p_max_results;
END;
$function$;
