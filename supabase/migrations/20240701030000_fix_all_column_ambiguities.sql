-- Migration to fix all column ambiguities in the hybrid_search function
-- This resolves the error: "column reference "title" is ambiguous"

-- Fix the hybrid_search function by properly qualifying all column references 
...
BEGIN
  ...
  WITH text_search AS (
    ...
    SELECT 
      ...
      p.title AS result_title,        -- FIX: Renamed to avoid ambiguity
      p.content AS result_content,    -- FIX: Renamed to avoid ambiguity
      p.url AS result_url,            -- FIX: Renamed to avoid ambiguity
      p.subreddit AS result_subreddit, -- FIX: Renamed to avoid ambiguity
      p.author_id AS author,
      p.created_at AS result_created_at, -- FIX: Renamed to avoid ambiguity
    ...
    SELECT 
      ...
      p.title AS result_title,        -- FIX: Renamed to avoid ambiguity
      c.content AS result_content,    -- FIX: Renamed to avoid ambiguity
      p.url AS result_url,            -- FIX: Renamed to avoid ambiguity
      p.subreddit AS result_subreddit, -- FIX: Renamed to avoid ambiguity
      c.author_id AS author,
      c.created_at AS result_created_at, -- FIX: Renamed to avoid ambiguity
  ...
  vector_search AS (
    SELECT 
      ts.parent_id,
      ts.content_type AS result_content_type, -- Renamed to avoid ambiguity
      ts.result_title,                 -- FIX: Use renamed column
      ts.result_content,               -- FIX: Use renamed column
      ts.result_url,                   -- FIX: Use renamed column
      ts.result_subreddit,             -- FIX: Use renamed column
      ts.author,
      ts.result_created_at,            -- FIX: Use renamed column
      CASE
  ...
  combined_scores AS (
    SELECT
      ...
      result_title AS title,           -- FIX: Use renamed column
      result_content AS content,       -- FIX: Use renamed column
      result_url AS url,               -- FIX: Use renamed column
      result_subreddit AS subreddit,   -- FIX: Use renamed column
      author,
      result_created_at AS created_at, -- FIX: Use renamed column
      vector_score,
      text_score,
...
-- Update the timed_hybrid_search function to match (unchanged except for calling the updated hybrid_search)
...
COMMENT ON FUNCTION public.hybrid_search IS 'Hybrid search function that combines PostgreSQL full-text search with vector similarity search. Fixed to properly handle column references, parameter interpolation, and all ambiguous column names.';
