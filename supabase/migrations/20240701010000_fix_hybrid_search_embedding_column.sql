-- Migration to fix the column reference in the hybrid_search function
-- This resolves the error: "column cr.embedding_512 does not exist"

-- Fix the hybrid_search function by updating the embedding column reference
    ...
    SELECT 
      ...
      CASE
        WHEN cr.representation_type = 'title' THEN 
          -- FIX: Using correct column name 'embedding' instead of 'embedding_512'
...
COMMENT ON FUNCTION public.hybrid_search IS 'Hybrid search function that combines PostgreSQL full-text search with vector similarity search. Fixed to properly handle column references and parameter interpolation.';
