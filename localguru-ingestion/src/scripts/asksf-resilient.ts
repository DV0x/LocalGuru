...
async function fetchResilient(): Promise<void> {
  ...
  try {
    ...
    for (let quarterIndex = 0; quarterIndex < QUARTERS.length; quarterIndex++) {
      ...
      console.log(`\n\n🔄 Starting to process quarter ${quarterIndex + 1}/${QUARTERS.length}: ${selectedQuarter.name}`);
      ...
      try {
        ...
        console.error(`\n❌ Failed to process quarter ${selectedQuarter.name}: ${errorMessage}`);
    ...
    console.log('\n\n✅✅✅ Completed processing all quarters ✅✅✅');
