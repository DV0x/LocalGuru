/**
 * AskSF Last Month Fetching Script
 * 
 * This script fetches data from the AskSF subreddit for the last month
...
// Create logger
const logger = new Logger('AskSFLastMonth');
...
// Define quarters to fetch - focusing on one month of data with different sort methods
const QUARTERS = [
  // Last month data with different sort methods
  { name: 'New (Last Month)', sort: 'new', limit: 1000, fetchAllTime: false, maxAgeHours: 24 * 30 },
  { name: 'Top (Last Month)', sort: 'top', limit: 1000, fetchAllTime: false, maxAgeHours: 24 * 30 },
  { name: 'Hot (Last Month)', sort: 'hot', limit: 1000, fetchAllTime: false, maxAgeHours: 24 * 30 },
  { name: 'Best (Last Month)', sort: 'best' as any, limit: 1000, fetchAllTime: false, maxAgeHours: 24 * 30 },
  { name: 'Controversial (Last Month)', sort: 'controversial' as any, limit: 1000, fetchAllTime: false, maxAgeHours: 24 * 30 },
  { name: 'Rising (Last Month)', sort: 'rising' as any, limit: 300, fetchAllTime: false, maxAgeHours: 24 * 30 },
] as const;
...
async function fetchResilient(): Promise<void> {
  logger.info('Starting resilient AskSF last month data fetch');
  console.log('\n🚀 STARTING RESILIENT ASKSF DATA FETCH (LAST MONTH)');
  console.log('═════════════════════════════════════════════════════');
  console.log(`📊 Processing ${QUARTERS.length} segments of last month Reddit data from r/${SUBREDDIT}`);
  console.log('═════════════════════════════════════════════════════\n');
